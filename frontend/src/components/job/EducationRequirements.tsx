...
export const EducationRequirements: React.FC<EducationRequirementsProps> = ({
  ...
  const convertGrade = (gradeValue: number, fromType: string, toType: string) => {
    ...
    if (toType === 'cgpa_10') return percentage / 10;
    if (toType === 'gpa_4') return (percentage / 100) * 4;

    return percentage;
  };

  const handleMinQualificationChange = (
    level: MinimumQualificationConfig['level']
  ) => {
    onChange({
      ...value,
  ...
  const handleMinGradeChange = (field: keyof GradeConfig, gradeValue: any) => {
    const currentGrade =
      value.minimum_qualification?.grade || {
        required: false,
  ...
  const handleDegreeRequirementChange = (
    field: keyof DegreeRequirementConfig,
    degreeValue: any
  ) => {
    onChange({
      ...value,
  ...
  const handleDegreeGradeChange = (field: keyof GradeConfig, gradeValue: any) => {
    const currentGrade =
      value.degree_requirement?.grade || {
        required: true,
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    { value: '12th_diploma', label: '12th OR Diploma' },
                    ...
                    <label
                      key={option.value}
                      className="flex items-center space-x-3 p-3 rounded-xl bg-white/5 hover:bg-white/10"
                    >
                      <input
                        type="radio"
              ...
              {(value.minimum_qualification?.level === '10th' ||
                ...
                <div className="p-4 bg-white/5 rounded-xl border border-white/20">
                  <h4 className="text-white font-semibold mb-3">Minimum Grade (Optional)</h4>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          >
                            ...
                            <option value="cgpa_10">CGPA (0–10)</option>
                            <option value="gpa_4">GPA (0–4)</option>
                          </select>
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              handleMinGradeChange('value', e.target.value)
                            }
                            className="w-full px-4 py-2 rounded-lg bg-white/10 border"
                          />
                        </div>
                      ...
                      {value.minimum_qualification.grade.value > 0 && (
                        <div className="p-3 bg-indigo-500/10 border border-indigo-500/30 rounded-lg">
                          <p className="text-indigo-200 text-sm">
                            ...
                            {getGradeEquivalents(
                              parseFloat(
                                String(value.minimum_qualification.grade.value)
                              ) || 0,
                ...
                <>
                  ...
                  <div>
                    ...
                    {value.degree_requirement.fields.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-3">
                        {value.degree_requirement.fields.map((field) => (
                          <span
                            key={field}
                            className="px-3 py-1 bg-indigo-500/20 border text-indigo-200 rounded-lg flex items-center"
                          >
                            {field}
                            <button
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2 rounded-lg bg-white/10 border mb-2"
                    >
                    ...
                    <div className="flex space-x-2">
                      <input
                        ...
                        className="flex-1 px-4 py-2 rounded-lg bg-white/10 border"
                      />
                      <GlassButton variant="secondary" onClick={() => addField(newField)}>
                        Add
                      </GlassButton>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center space-x-3 p-3 bg-white/5 rounded-xl">
                    ...
                    <label htmlFor="accept-related" className="text-white/90 text-sm">
                      ...
                      <span className="text-white/60 text-xs">
                        (Example: Software Engineering ↔ Computer Science)
                      </span>
                    </label>
                  </div>
                    ...
                    <div className="space-y-3">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <select
                            value={value.degree_requirement.grade?.type || 'cgpa_10'}
                      ...
                      {(value.degree_requirement.grade?.value || 0) > 0 && (
                        <div className="p-3 bg-indigo-500/10 border border-indigo-500/30 rounded-lg">
                          <p className="text-indigo-200 text-sm">
                            ...
                            {getGradeEquivalents(
                              parseFloat(
                                String(value.degree_requirement.grade?.value)
                              ) || 0,
              ...
              {value.alternative_paths?.experience_substitute?.enabled && (
                <div className="ml-7">
                  ...
                  <input
                    ...
                    className="w-32 px-4 py-2 rounded-lg bg-white/10 border"
                  />

                  <p className="text-white/60 text-sm mt-2">
                    Candidates with{' '}
                    {value.alternative_paths.experience_substitute.years_required}
                    + years of relevant experience can substitute for degree
                    requirements.
                  </p>
                </div>
              )}
