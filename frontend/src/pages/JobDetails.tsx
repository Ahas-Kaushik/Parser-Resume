...
import { MapPin, Briefcase, DollarSign, Calendar, Building2, ArrowLeft, FileText } from 'lucide-react';
...
export default function JobDetails() {
  ...
  const handleApply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.current_position) formDataToSend.append('current_position', formData.current_position);
      if (formData.current_salary) formDataToSend.append('current_salary', formData.current_salary);
                ...
                {applications.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {applications.map((app) => (
                      <div
                        key={app.id}
                        className="p-4 bg-white/5 rounded-xl border border-white/20"
                      >
                        <div className="flex items-start justify-between">
                          ...
                          <div className="text-right">
                            <div className={`text-2xl font-bold ${app.score ? (app.score >= 70 ? 'text-green-400' : 'text-red-400') : 'text-white/50'}`}>
                              {app.score ? `${app.score.toFixed(1)}/100` : 'N/A'}
                            </div>
                            <div className={`text-sm px-2 py-1 rounded ${app.status === 'selected' ? 'bg-green-500/20 text-green-300' : app.status === 'rejected' ? 'bg-red-500/20 text-red-300' : 'bg-yellow-500/20 text-yellow-300'}`}>
                              {app.status}
                            </div>
                          </div>
                        </div>
        ...
        <form onSubmit={handleApply} className="space-y-6">
          ...
          <div className="p-4 bg-blue-500/20 border border-blue-500/50 rounded-xl">
            <p className="text-sm text-blue-200">
              <strong>Note:</strong> Your resume will be instantly analyzed by our AI. 
              You'll receive feedback within seconds!
            </p>
          </div>
